...
class GA():
        ...
        def crossing(self, first, second, th):
            for i in range(self.count_state):
                if random.random() < self.coef_c:
                    ...
                    first[i][0][0] = th[i][0][0]
                if random.random() < self.coef_c:
                    first[i][0][1] = second[i][0][1]
                elif random.random() < self.coef_c:
                    first[i][0][1] = th[i][0][1]
                if random.random() < self.coef_c:
                    first[i][1][0] = second[i][1][0]
                elif random.random() < self.coef_c:
                    first[i][1][0] = th[i][1][0]
            return first
        ...
        def get_new_population(self, population):
            ...
            for i in range(self.population_count):
                first, second, th = random.choices(list(population), k=3)#Из группы лучших решений случайным образом выбирается пара решений.
                crossing = self.crossing(first, second, th) # Формируется новое решение с помощью применения оператора скрещивания [4] к двум выбранным решениям.
        ...
        def run(self):
            pre_score = 0
            current_population = np.array([self.initialization() for _ in range(self.population_count)])
            for e in range(self.iter):
                ...
                for i in range(self.population_count):
                    ...
                    new_population_score_list.append((new_population_score, new_population_step, new_population[i]))
                    current_population_score_list.append((current_population_score, current_population_step, current_population[i]))
                ...
                for score, _, individual in combined_scores:
                    if score_counts[score] < 2 and id(individual) not in added_individuals:
                        selected_population.append(individual)
                        added_individuals.add(id(individual))
                        score_counts[score] += 1
                    if len(selected_population) == self.population_count:
                        break
                ...
                best_score, _ = self.get_best_individuals(current_population)
                if best_score > pre_score:
                    pre_score = best_score
                    print(f"Epoch: {e + 1}, best_score: {best_score}, time: {time.time() - start_time}")
